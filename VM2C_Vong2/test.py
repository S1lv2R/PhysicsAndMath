...
for i in range(len(t)):
    ...
    for z in range(len(old_indices)):
        c[old_indices[z, 0], old_indices[z, 1]] = 1
    for j in range(len(shifts)):
        ...
        for s in range(len(indices)):
            ...
            output += f"{start_date.strftime('%Y-%m-%d')} {start}:00:00 -> {end}:00:00 {shift} {vid} Day_chuyen_1 {dict[h[1]]}\n"
